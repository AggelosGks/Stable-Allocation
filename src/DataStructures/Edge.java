...
public class Edge {
	...
	public boolean isEqualReversed(Edge other, int jobs, int machines) {
		if (this.job.id == other.machine.id - jobs && this.machine.id == other.job.id + machines
	...
	/**
	 * Creates and stores all the edges of the bipartite graph. We indicate to
	 * an edge by the id of each node.
	 * 
	 */
	public static void createAllEdges() {
		for (JobNode job : JobNode.getJobs()) {// iterate job nodes
			...
			for (MachineNode machine : MachineNode.getMachines()) {
				jobs_machines.get(job.id).add(new Edge(job, machine));// add
																		// each
																		// edge
			}
			jobs_machines.get(job.id).add(new Edge(job, MachineNode.getDummy()));// add
																					// edge
																					// with
																					// dummy
																					// machine
		...
		for (MachineNode machine : MachineNode.getMachines()) {
			jobs_machines.get(dummy.id).add(new Edge(dummy, machine));// add
																		// each
																		// edge
		}
		jobs_machines.get(dummy.id).add(new Edge(dummy, MachineNode.getDummy()));// add
																					// each
																					// edge
	}
	...
	public String toString() {
		return "Edge [machine=" + machine.id + ", job=" + job.id + ", max_time=" + max_time + ", current_time="
				+ current_time + "]";
	}

	public String toStringSwaped() {
		int id = job.id + MachineNode.getMachines().size();
		return "Edge [machine=" + machine.id + ", job=" + job.id + "(M" + id + ")" + ", max_time=" + max_time
